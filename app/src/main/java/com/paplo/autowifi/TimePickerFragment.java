...
/**
 * Created by grajp on 22.03.2018.
 */
...
public class TimePickerFragment extends DialogFragment implements TimePickerDialog.OnTimeSetListener {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        } else if (getTag().contains("startTimePicker") && PickerActivity.startLong != 0){

            hour = (int) TimeUnit.MILLISECONDS.toHours(PickerActivity.startLong);
            long hourInMillis = TimeUnit.HOURS.toMillis(hour);
            minute = (int) TimeUnit.MILLISECONDS.toMinutes(PickerActivity.startLong - hourInMillis);

        } else if (getTag().contains("endTimePicker") && PickerActivity.startLong != 0){

            hour = (int) TimeUnit.MILLISECONDS.toHours(PickerActivity.endLong);
            long hourInMillis = TimeUnit.HOURS.toMillis(hour);
            minute = (int) TimeUnit.MILLISECONDS.toMinutes(PickerActivity.endLong - hourInMillis);
    ...
    public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
        ...
        if (getTag().contains("Detail")){
            ...
            String time  = DateUtils.formatDateRange(pickerActivity.getBaseContext(), f, timeInMillis, timeInMillis, DateUtils.FORMAT_SHOW_TIME, TimeZone.getDefault().toString()).toString();
            pickerActivity.setTimeTextViews(getTag(), time, timeInMillis);
        }
